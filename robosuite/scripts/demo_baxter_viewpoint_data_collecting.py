...
def place_objects_in_bin(env, num_objects=10):
    ...
    for i in range(num_objects):
        object_z = env.env.model.bin_size[2] + 0.10
        object_x = -0.00#0.1 * np.random.random() - 0.05
        object_y = 0.00#0.1 * np.random.random() - 0.05
...
if __name__ == "__main__":
    ...
    for i in range(0, num_episodes):
        ...
        if test == False:
            save_file_name = "data/vpn_grasp/episode_" + str(i) + ".npz"
            np.savez(save_file_name, vt_c_im_list, vt_d_im_list)
