...
class ResNet_ft:
    def __init__(self, task, action_size, model_name=None, batch_size=64, repetition = [3,4,6,3], task_num = [1,2,3]):
        ...
        self.checkpoint_dir = os.path.join(FILE_PATH, 'ft_train_log/', 'keras_saved_model/')
        self.model = None
        self.data_lists = {}
    ...
    def ResNetwork(self, _input_shape, output_dim, repetition=[3,3,4]):
        ...
        output_first = Dense(units=output_dim,
                        activation='softmax', name='output_first')(x_last)
        output_second = Dense(units=output_dim,
                        activation='softmax', name='output_second')(x_last)
        output_third = Dense(units=output_dim,
                        activation='softmax', name='output_third')(x_last)
    ...
    def set_datapath(self, task_num_old, task_num_new, task_num_newer,
        ...
        if not os.path.exists(os.path.join(FILE_PATH,'ft_train_log/failed',self.folder_name)):
            os.makedirs(os.path.join(FILE_PATH,'ft_train_log/failed',self.folder_name))
    ...
    def test_agent(self, num_rep = 10, task_num = 0, head = 0):
        ...
        for n in range(num_rep): #self.num_test_ep):
            ...
            while not done:
                ...
                action = np.argmax(self.model.predict(np.expand_dims(obs_re, axis=0))[head])
            ...
            if cumulative_reward >= 90:
                ...
                im1.save(os.path.join(FILE_PATH, 'ft_train_log', 'failed/'+self.folder_name+'/test_'
                                      + datetime.datetime.now().strftime("%m%d_%H%M%S")+'_'+str(stucked)+'-view1.png'))
                im2.save(os.path.join(FILE_PATH, 'ft_train_log', 'failed/'+self.folder_name+'/test_'
    ...
    def train(self, task_order=0):
        ...
        if task_order ==0:
            ...
            self.model.compile(loss ={'output_first':self.custom_loss1,
                                    'output_second':self.custom_loss0,
                                    'output_third':self.custom_loss0},
            ...
            self.model.compile(loss ={'output_first':self.custom_loss0,
                                    'output_second':self.custom_loss1,
                                    'output_third': self.custom_loss0},
                                    optimizer=optimizer, metrics= ['accuracy'],
            ...
            self.model.compile(loss ={'output_first':self.custom_loss0,
                                    'output_second':self.custom_loss0,
                                    'output_third':self.custom_loss1},
                                    optimizer=optimizer, metrics= ['accuracy'],
        ...
        writer = SummaryWriter(os.path.join(FILE_PATH, 'ft_train_log/', 'tensorboard', self.folder_name))
        sess.run(tf.global_variables_initializer())
        if not os.path.exists(self.checkpoint_dir):
            os.makedirs(self.checkpoint_dir)
        if not os.path.exists(os.path.join(FILE_PATH,'ft_train_log','failed')):
            os.makedirs(os.path.join(FILE_PATH, 'ft_train_log', 'failed'))
        ...
        for epoch in range(self.num_epochs+1):
            ...
            for round in range(num_rounds):
                ...
                if task_order ==0:
                    y= {'output_first':train_a_data, 'output_second': train_logit_data1, 'output_third':train_logit_data2}
                    met = ["output_first_loss", "output_first_acc", "loss"]
                    test_layer = ['output_first', 'output_second', 'output_third']
                elif task_order ==1:
                    y= {'output_second':train_a_data, 'output_first': train_logit_data1, 'output_third':train_logit_data2}
                    met = ["output_second_loss", "output_second_acc","loss"]
                    test_layer = ['output_first', 'output_second', 'output_third']
                elif task_order ==2:
                    y= {'output_third':train_a_data, 'output_first': train_logit_data1, 'output_second': train_logit_data2}
                    met = ["output_third_loss", "output_third_acc","loss"]
                    test_layer = ['output_first', 'output_second', 'output_third']
            ...
            if ep_train_acc > self.max_accur:
                self.model.save(os.path.join(FILE_PATH, 'ft_train_log/', 'keras_saved_model', folder_name+".h5"))
...
def main():
    ...
    env = IKWrapper(env)
     ...
     #'data/processed_data'
    model = ResNet_ft(task, 10, model_name='ResNet', batch_size=batch_size, repetition =[3,4,6,3])
