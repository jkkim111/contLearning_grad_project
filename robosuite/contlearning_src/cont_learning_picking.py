...
class BaxterTestingEnv1():
    def __init__(self, env, task='pick', continuous=False,
                 ...
                 random_spawn=True, rgbd=False, block_random = True, print_on=False, action_type='2D', \
                 viewpoint1 = 'wholeview',viewpoint2 = 'rlview2',viewpoint3 = "birdview", grasping_env = 1,
                 down_level = 2, is_test = True, arm_near_block=False,
                 only_above_block = False, down_grasp_combined = False, mov_dist = 0.02, object_type = 'smallcube'):
        ...
        self.arm_near_block = arm_near_block


        self.mov_dist = mov_dist if self.task == 'pick' else 0.04
        self.max_step = 30 # int(1.60/self.mov_dist)

        self.linger_mem = [0]*10
    ...
    def reset(self):
        ...
        self.step_count = 0
        self.linger_mem = [0]*len(self.linger_mem)
        ...
        # randomly spawn until goal and main_pos > 0.15
        faraway = np.random.uniform(0.0, 1.0)
        if self.block_random:
            while True:
                spawn_count += 1
            ...
            self.main_pos = self.arena_pos + np.array([0.0, 0.0, 0.05])
        ...
        if self.task=='reach':
            if self.action_type=='2D':
                self.state[8] = self.arena_pos[2] + 0.08 * np.random.uniform(low=0.47, high=1.0)
                if self.random_spawn:
                    self.state[6:8] = self.arena_pos[:2] + spawn_range * np.random.uniform(low=-1.0, high=1.0, size=2)
                else:
                ...
                if self.random_spawn:
                    self.state[6:8] = self.arena_pos[:2] + spawn_range * np.random.uniform(low=-1.0, high=1.0, size=2)
                    self.state[8] = self.arena_pos[2] + 0.14 + 0.06 * np.random.uniform(low=-1.0, high=1.0)
                else:
                    self.state[6:8] = self.arena_pos[:2]
                    self.state[8] = self.arena_pos[2] + 0.14  # 0.16
                # if self.random_spawn:
        ...
        #2. Push
            ...
            self.state[11] = -np.argmin(align_angle_gripper)*np.pi/4*1.5 # -align_angle #

            #initial position of (x,y,z) = colinear with object and target.

            if self.action_type == '2D':
                if self.random_spawn:
                    self.state[8] = self.arena_pos[2] + np.random.uniform(low=0.47, high=1.0) * 0.075
                else:
                    self.state[8] = self.arena_pos[2] + 0.055

            elif self.action_type == '3D':
                if self.random_spawn:
                    self.state[8] = self.arena_pos[2] + 0.14 + 0.04 * np.random.uniform(low=-1.0, high=1.0)
                else:
                    self.state[8] = self.arena_pos[2] + 0.14
        ...
        #3. Pick
            ...
            if self.random_spawn:
                while True:
                    ...
                    # print("block random : (%.3f,%.3f)"%(self.main_pos[0]-self.arena_pos[0],self.main_pos[1]-self.arena_pos[1]))
                    # print("arm random : (%.3f,%.3f)"%(self.state[6]-self.arena_pos[0],self.state[7]-self.arena_pos[1]))
                    # print("block-center:%.3f"%(np.linalg.norm((self.main_pos[0]-self.arena_pos[0],self.main_pos[1]-self.arena_pos[1]))))
                    # print("arm-center:%.3f"%(np.linalg.norm((self.state[6]-self.arena_pos[0],self.state[7]-self.arena_pos[1]))))
                    # print("arm-block:%.3f"%(np.linalg.norm((self.state[6]-self.main_pos[0],self.state[7]-self.main_pos[1]))))

                    if  (np.linalg.norm(self.state[6:8] - self.main_pos[:2]) > self.spawn_range): #(np.linalg.norm(randomness) > 0.05) and
                        break
                # if self.down_level == 1:
                #     self.state[8] = self.arena_pos[2] + np.random.uniform(low=0.12, high=0.22)
                ...
                while True:
                    randomness = (self.mov_dist) * np.random.uniform(low=-1.0, high=1.0, size=2)
                    if np.linalg.norm(randomness) < self.mov_dist:
                        break
                # self.state[6:8] = self.main_pos[:2]
                self.state[6:8] = self.main_pos[:2] + randomness
                self.state[8] = self.arena_pos[2] + 0.22

            elif self.arm_near_block:
                while True:
                    randomness = (spawn_range) * np.random.uniform(low=-1.0, high=1.0, size=2)
                    self.state[6:8] = self.arena_pos[:2] + randomness
                    if np.linalg.norm(randomness) < self.spawn_range * 0.3 :
                        break
                self.state[8] = self.arena_pos[2] + 0.22
                ...
                # #     self.state[6:8] = self.arena_pos[:2] + (self.main_pos[:2] - self.arena_pos[:2])*1.3
                # #     while True:
                # #         randomness = (spawn_range + 0.02) * np.random.uniform(low=-1.0, high=1.0, size=2)
                # #         self.state[6:8] = self.arena_pos[:2] + randomness
                ...
                # # else:
                # #   self.state[6:8] = self.arena_pos[:2]
                # if self.arm_near_block and not self.is_test:
                #
                # randomness = (spawn_range) * np.random.uniform(low=-1.0, high=1.0, size=2)
                self.state[6:8] = self.arena_pos[:2]
    ...
    def step(self, action):
        ...
        # print('arena pos : ' , self.arena_pos)
        self.curr_main_pos = np.copy(self.env.sim.data.body_xpos[self.obj_id])
        ...
        if self.arm_pos[0] < self.arena_pos[0]-self.spawn_range-0.05  or self.arm_pos[0] > self.arena_pos[0]+self.spawn_range+0.05 :
            print('x-axis out of bound')
            stucked = -2
        elif self.arm_pos[1] < self.arena_pos[1]-self.spawn_range-0.05 or self.arm_pos[1] > self.arena_pos[1]+self.spawn_range+.05:
    ...
    ####PICK RL -> When to decide the end of Episode
            ...
            # print("step",self.step_count," - linger_mem is: ", self.linger_mem)
            if self.step_count < 5 :
                pass
            else:
                self.linger_mem[:-1] = self.linger_mem[1:]
                if np.linalg.norm(self.state[6:8]-self.curr_main_pos[0:2])< self.mov_dist :
                    self.linger_mem[-1] = 1
                else:
                    self.linger_mem[-1] = 0

            if np.sum(self.linger_mem) > 5 and self.linger_mem[-1] == 0:
                stucked = -3

            elif (self.linger_mem == [1]*len(self.linger_mem)) and action != 9 :
                print("lingering -> will go down!")
                action = 9
                if action == 9:
                    if self.down_grasp_combined:
                        self.arm_pos = self.arm_pos + np.array([0.0, 0.0, (self.curr_main_pos[2]-self.arm_pos[2])]) #-mov_dist])
            ...
            if action == 9:
                ...
                if self.down_grasp_combined:
                    if self.check_grasp():
                        ...
                        print('episode done. [SUCCESS] by action down')
                        stucked = 0
                    else:
                        reward = 0.0
                        done = True
            ...
            if stucked == -1 : # or modify_stucked(arm_euler) :
                ...
                print('episode done. [lingered then GOT AWAY]')
                ...
                if action== 8:
                    if self.check_grasp():
                        ...
                        print('episode done. [SUCCESS] by action grasping')
                        stucked = 0
                    else:
                        reward = 0.0
                        done = True
