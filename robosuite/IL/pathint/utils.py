...
def construct_continual_mini_imagenet(split='train'):
    pickle_file = '../dataset/mini-imagenet-cache-train.pkl'
    f = open(pickle_file, 'rb')
    data1 = pickle.load(f)
    pickle_file = '../dataset/mini-imagenet-cache-val.pkl'
    f = open(pickle_file, 'rb')
    data2 = pickle.load(f)
    pickle_file = '../dataset/mini-imagenet-cache-test.pkl'
    f = open(pickle_file, 'rb')
    data3 = pickle.load(f)

    X = np.concatenate((data1['image_data'], data2['image_data'], data3['image_data']), axis=0)
    X = X.astype('float32')
    X = X / 255.0
    size = np.shape(X)[0]
    y = np.array(range(size)).reshape(-1)
    if split == 'train':
        temp_idx = (y % 600 < 500)
        num_c = 500
    else:
    ...
    datasets = []
    for i in range(10):
        data = X[i * num_c * 10:(i + 1) * num_c * 10, ...], np_utils.to_categorical(y[i * num_c * 10:(i + 1) * num_c * 10].reshape(-1, 1), 100)
        datasets.append(data)

    # all_task_labels = []
    # all_task_labels.extend([range(10 * i, 10 * (i + 1)) for i in range(10)])
