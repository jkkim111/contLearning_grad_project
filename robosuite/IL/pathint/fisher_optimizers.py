...
from pathint.regularizers import quadratic_regularizer, quadratic_regularizer_2
...
class KOOptimizer(Optimizer):
    ...
    def __init__(self, opt, step_updates=[], task_updates=[], init_updates=[], task_metrics={}, regularizer_fn=quadratic_regularizer_2,
        ...
        if not isinstance(task_updates, OrderedDict):
            task_updates = OrderedDict(task_updates)
        if not isinstance(init_updates, OrderedDict):
            init_updates = OrderedDict(init_updates)
        # task_metrics
    ...
    def get_updates(self, params,loss,model=None):
        ...
        self._fishers = compute_fishers(self.model)
        # fishers = compute_fisher_information(model)
        self.vars['fishers'] = dict(zip(self.weights, self._fishers))
